...
from igpy.util_torch.helpers import TorchDevice, get_all_torch_devices, get_canonical_torch_device
...
class ImageTensor:
    ...
    def _check_tensor(self, attribute, value : torch.Tensor | None):
        ...
        assert value.dtype in (torch.uint8, torch.float16, torch.float32), f'unsupported dtype {value.dtype}'
        
    @layout.validator
    def _check_layout(self, attribute, value : str):
        assert value is not None, 'layout must be specified'
        assert value in ('nchw', 'nhwc'), f'unsupported layout {value}, must be nchw or nhwc'
...
class SchedulerTypes(CustomEnumBase):
    ...
    def get_prefer_order(cls) -> list[str]:
        ...
        return [cls.EulerDiscrete, cls.DPM_2M_Karras, cls.DPM_2M, 
                cls.DPM_2M_SDE, cls.DPM_2M_SDE_Karras]
...
class SDModelConfig:
    ...
    def _check_primary_model_key(self, attribute, value):
        assert isinstance(value, str), f'primary_model_key must be str, got {type(value)}'
        is_sd_15 : bool = value in SDModelKeys.get_enum_values()
        is_sdxl : bool = value in SDXLModelKeys.get_enum_values()
        assert is_sd_15 or is_sdxl, f'primary_model_key must be one of {SDModelKeys.get_enum_values()} or {SDXLModelKeys.get_enum_values()}, got {value}'
...
class ModelConfigs:
    ...
    DefaultSchedulerType_sd_v15 : ClassVar[str] = field(default=SchedulerTypes.DPM_2M_Karras)
    DefaultSchedulerType_sdxl : ClassVar[str] = field(default=SchedulerTypes.EulerDiscrete)
    ...
    def set_base_model_path(self, model_key : str, path : str):
        ...
        '''
        import os
        all_model_keys = SDModelKeys.get_enum_values() + \
        ...
        for sdkey in SDModelKeys.get_enum_values() + SDXLModelKeys.get_enum_values():
            ...
            for ff in fields(mdconfig.__class__):
                ...
                if isinstance(fval, TextModelComponent):
                    if fval.encoder.path is not None and fval.encoder.key not in all_paths:
                        assert os.path.exists(fval.encoder.path), f'path {fval.encoder.path} does not exist'
                        all_paths[fval.encoder.key] = fval.encoder.path
                    if fval.tokenizer.path is not None and fval.tokenizer.key not in all_paths:
                        assert os.path.exists(fval.tokenizer.path), f'path {fval.tokenizer.path} does not exist'
                        all_paths[fval.tokenizer.key] = fval.tokenizer.path

                elif isinstance(fval, ModelComponent) and fval.path is not None and fval.key not in all_paths:
    ...
    def get_model_config(self, model_key : str) -> SDModelConfig:
        ...
        if model_key in SDModelKeys.get_enum_values():  # this is 1.5
            if self.use_shared_model_components:
                SharedModel = SDModelKeys.get_base_model()  # we expect all sd models share some components with base model
            else:
            ...
            mdconfig = SDModelConfig(
                ...
                text_model= TextModelComponent(
                    ...
                    encoder=ModelComponent(key=SharedModel + '.' + TextModelKeys.CLIP_TextEncoder_prompt_1),
                    tokenizer=ModelComponent(key=SharedModel + '.' + TextModelKeys.CLIP_TextTokenizer_prompt_1),
                ),
                unet = ModelComponent(key=model_key + '.unet'),
                vae = ModelComponent(key=SharedModel + '.vae'),
                scheduler= SchedulerComponent(key=model_key + '.scheduler',
            ...
            mdconfig = SDModelConfig(
                ...
                text_model_2 = TextModelComponent(
                    ...
                    encoder=ModelComponent(key=SharedModel + '.' + TextModelKeys.CLIP_TextEncoder_prompt_2),
                    tokenizer=ModelComponent(key=SharedModel + '.' + TextModelKeys.CLIP_TextTokenizer_prompt_2),
                ),
                unet = ModelComponent(key=model_key + '.unet'),
                vae = ModelComponent(key=SharedModel + '.vae'),
                scheduler= SchedulerComponent(key=model_key + '.scheduler',
    ...
    def get_default_model_root(cls) -> str:
        ...
        return str(pathlib.Path(__file__).parent.parent.absolute() / "data/models").replace('\\', '/')
    
    @classmethod
    def get_default_bento_model_dir(cls) -> str:
        return str(pathlib.Path(__file__).parent.parent.absolute() / "data/bento-models").replace('\\', '/')
...
class Logme(CustomLogger):
    ...
    def __init__(self) -> None:
        super().__init__(logger_name=self._default_logger_name, 
