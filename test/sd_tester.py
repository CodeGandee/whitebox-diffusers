...
from pyaigc.loader.LocalModelLoader import LocalModelLoader, DiffusionModelBundle
...
class InputCase:
    ...
    # the image will be added noise at this timestep index, for image-to-image generation
    # if None, a new random latent will be created, without using the content of the image
    init_noise_timestep_index: int | None = field(default=None)

    # image data
    # for text-to-image generation, only the image size is used, the content will be ignored
    # for image-to-image generation, the image content will be used
    ...
    def __attrs_post_init__(self):
        assert (
            self.fn_image is not None or self.image is not None
    ...
    def crop_by(self, xmin: int, ymin: int, w: int, h: int):
        ...
        if self.openpose_image is not None:
            self.openpose_image = self.openpose_image[
                ymin : ymin + h, xmin : xmin + w, ...
            ]
    ...
    def resize_to(self, h: int, w: int, divisible_by: int = None):
        ...
        if self.canny_image is not None:
            self.canny_image = ip.imresize(
                self.canny_image, (h, w), interp_method="nearest"
            )

        if self.depth_image is not None:
            self.depth_image = ip.imresize(
                self.depth_image, (h, w), interp_method="nearest"
            )

        if self.openpose_image is not None:
            self.openpose_image = ip.imresize(
                self.openpose_image, (h, w), interp_method="nearest"
            )
...
class PipelineConfig:
    ...
    def __attrs_post_init__(self):
        if self.ip_adapters:
            assert (
                self.with_ip_image_encoder
            ), "ip_image_encoder must be enabled when ip_adapters is given"


@define(kw_only=True, eq=False)
class SDTester:
    model_pipeline: SDPipeline = field(default=None)
    common_positive_prompt: str = field(
        default="masterpiece, high quality, very sharp, photorealistic"
    )
    common_negative_prompt: str = field(
        default="bad face, blurry, low quality, gray, dull, lack of detail, naked"
    )
    ...
    def load_model_pipeline(
    ...
    ) -> tuple[SDPipeline, DiffusionModelBundle]:

        # load models
        model_config = C.ModelConfigs.get_instance().get_model_config(
            config.sd_model_key
        )
        ...
        for info, name in [
            (model_config.controlnets.canny, "canny"),
    ...
    def _create_diffusion_state(self, target_width: int, target_height: int) -> SDState:
        dstate = SDState(target_height=target_height, target_width=target_width)
        return dstate

    def denoise_dstate(
        self,
        dstate_list: list[SDState],
        num_step: int = None,
        ...
        for it in tqdm(range(num_steps_required)):
            ...
            for dstate in dstate_list:
                if (
                    dstate.scheduler.next_step_index is not None
                    and dstate.is_inpainting
                    and apply_latent_mask
                ):
                    dstate.latent = dstate.get_latent_for_inpainting(latent_noise=noise)

            should_continue = pline.step_denoise(
                dstate_list, apply_latent_mask=apply_latent_mask
            )
    ...
    def add_noise_to_dstate(
        ...
        if noise is None:
            ...
            noise = torch.randn(lt.shape, generator=rng).to(
                dtype=lt.dtype, device=lt.device
            )
        else:
        ...
        # dstate.latent = ltnew
        dstate.add_masked_noise_to_latent(
            noise,
            to_timestep_index=to_timestep_index,
        ...
        if prepare_for_denoise:
            self.model_pipeline.init_diffusion_state(
                dstate, reset_latent=False, reset_timestep=True
            )
            dstate.set_timestep_sequence_by_num_steps(
                num_denoise_step, max_timestep_index=to_timestep_index
            )
    ...
    def init_output_case(self, input_case: InputCase) -> OutputCase:
        ...
        if (
            input_case.positive_prompt is not None
            and len(input_case.positive_prompt) > 0
        ):
            dstate.positive_prompt.text = (
                input_case.positive_prompt + "," + self.common_positive_prompt
            )
        else:
            dstate.positive_prompt.text = self.common_positive_prompt

        if (
            input_case.negative_prompt is not None
            and len(input_case.negative_prompt) > 0
        ):
            dstate.negative_prompt.text = (
                input_case.negative_prompt + "," + self.common_negative_prompt
            )
        else:
        ...
        if input_case.canny_weight > 0 and input_case.canny_image is not None:
            canny_image = input_case.canny_image
            # Ensure canny image is single channel
            if canny_image.ndim == 3 and canny_image.shape[2] > 1:
                canny_image = canny_image[..., 0]  # Take first channel if RGB
            dstate.controlnet_data["canny"] = ControlnetData(
                image_data=canny_image[None, ..., None],  # Add batch and channel dims
        ...
        if input_case.depth_weight > 0 and input_case.depth_image is not None:
            ...
            dstate.controlnet_data["depth"] = ControlnetData(
                image_data=depth_image[None, ..., None],
                image_data_layout="nhwc",
        ...
        if input_case.ip_adapter_images:
            for name, ipa_image in input_case.ip_adapter_images.items():
                dstate.image_prompt_data[name] = ImagePromptData(
                    name=name, image=ipa_image
                )

        # init the diffusion state
        if (
            input_case.init_noise_timestep_index is not None
            and input_case.image is not None
        ):
            # Ensure image is 3 channel or 1 channel
            if input_case.image.ndim == 3 and input_case.image.shape[2] not in [1, 3]:
                raise ValueError(
                    f"Image must have 1 or 3 channels, got {input_case.image.shape[2]} channels"
                )
            elif input_case.image.ndim != 2 and input_case.image.ndim != 3:
                raise ValueError(
                    f"Image must be 2D (single channel) or 3D (with channels), got shape {input_case.image.shape}"
                )

            # Convert uint8 image to float32 with values between (0,1)
            if (
                input_case.image.dtype == np.uint8
                or input_case.image.dtype == np.uint16
                or input_case.image.dtype == np.uint32
            ):
                image_tensor = (
                    torch.from_numpy(input_case.image)
                    .float()
                    .div(255.0)
                ...
                image_tensor = (
                    torch.from_numpy(input_case.image).float().to(self.target_device)
                )

            # Handle both 1-channel and 3-channel images, ensuring proper NHWC format
            if image_tensor.ndim == 2:  # Single channel image (H, W)
                image_tensor = image_tensor.unsqueeze(
                    -1
                )  # Add channel dimension (H, W, 1)

            # If batch size > 1, repeat the image to create a batch
            if input_case.batch_size > 1:
                image_tensor = image_tensor.unsqueeze(0).repeat(
                    input_case.batch_size, 1, 1, 1
                )
            else:
                image_tensor = image_tensor.unsqueeze(
                    0
            ...
            dstate.latent = self.model_pipeline.encode_image_to_latent(
                image_tensor, input_data_layout="nhwc", force_upcast=None
            )

            # Initialize other aspects of the diffusion state
            self.model_pipeline.init_diffusion_state(
                dstate,
                reset_latent=False,  # Don't reset latent as we just set it
                reset_timestep=True,
            ...
            # add noise for image-to-image generation
            self.add_noise_to_dstate(
                dstate,
                to_timestep_index=input_case.init_noise_timestep_index,
                num_denoise_step=input_case.num_denoise_step,
                noise=None,
                apply_latent_mask=False,
                prepare_for_denoise=True,
            )
        else:
            self.model_pipeline.init_diffusion_state(
                dstate,
